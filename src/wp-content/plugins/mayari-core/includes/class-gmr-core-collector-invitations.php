...
final class GMR_Core_Collector_Invitations {
	...
	public static function admin_assets( string $hook ): void {
		...
		wp_enqueue_style( 'gmr-collector-invitation-filters', plugins_url( 'assets/admin-invitations-filters.css', GMR_CORE_FILE ), array( 'gmr-collector-invitations' ), GMR_CORE_VERSION );
	...
	public static function revoke_invitation(): void {
		...
		set_transient( 'gmr_collector_invitation_revoked_' . get_current_user_id(), true, MINUTE_IN_SECONDS );
		wp_safe_redirect( admin_url( 'users.php?page=gmr-collector-invitations' ) ); exit;
	}

	public static function admin_page(): void {
		if ( ! self::can_manage() ) return;
		global $wpdb; $all_invites = $wpdb->get_results( 'SELECT * FROM ' . self::table() . ' ORDER BY id DESC LIMIT 100' );
		$filter = sanitize_key( wp_unslash( $_GET['filter'] ?? 'current' ) );
		if ( ! in_array( $filter, array( 'current', 'active', 'used', 'expired', 'revoked' ), true ) ) $filter = 'current';
		$counts = array( 'current'=>0, 'active'=>0, 'used'=>0, 'expired'=>0, 'revoked'=>0 );
		foreach ( $all_invites as $entry ) { $entry_state = self::state( $entry ); if ( 'revoked' !== $entry_state ) $counts['current']++; $counts[ $entry_state ]++; }
		$invites = array_values( array_filter( $all_invites, static function( object $entry ) use ( $filter ): bool { $entry_state = self::state( $entry ); return 'current' === $filter ? 'revoked' !== $entry_state : $filter === $entry_state; } ) );
		...
		$revoked = get_transient( 'gmr_collector_invitation_revoked_' . get_current_user_id() ); if ( $revoked ) delete_transient( 'gmr_collector_invitation_revoked_' . get_current_user_id() );
		?>
		<div class="wrap gmr-invitations">
			...
			<?php if ( $created ) : ?><section class="gmr-invitations__created"><span><?php esc_html_e( 'INVITACIÓN CREADA', 'mayari-core' ); ?></span><h2><?php esc_html_e( 'Lista para compartir', 'mayari-core' ); ?></h2><div><code><?php echo esc_html( $created['url'] ); ?></code><button class="button button-primary" type="button" data-gmr-copy="<?php echo esc_attr( $created['url'] ); ?>"><?php esc_html_e( 'Copiar enlace', 'mayari-core' ); ?></button></div></section><?php elseif ( $revoked ) : ?><div class="notice notice-success is-dismissible"><p><?php esc_html_e( 'La invitación fue revocada.', 'mayari-core' ); ?></p></div><?php endif; ?>
			...
			<section class="gmr-invitations__history"><div class="gmr-invitations__section-head"><div><span>02 · SEGUIMIENTO</span><h2><?php esc_html_e( 'Invitaciones recientes', 'mayari-core' ); ?></h2></div><p><?php esc_html_e( 'Por defecto se ocultan las invitaciones revocadas; el historial completo sigue disponible aquí.', 'mayari-core' ); ?></p></div><nav class="gmr-invitations__filters" aria-label="Filtrar invitaciones"><?php foreach ( array( 'current'=>'No revocadas', 'active'=>'Pendientes', 'used'=>'Registradas', 'expired'=>'Vencidas', 'revoked'=>'Revocadas' ) as $key => $label ) : ?><a class="<?php echo $filter === $key ? 'is-current' : ''; ?>" href="<?php echo esc_url( add_query_arg( 'filter', $key, admin_url( 'users.php?page=gmr-collector-invitations' ) ) ); ?>"><?php echo esc_html( $label ); ?> <b><?php echo esc_html( $counts[ $key ] ); ?></b></a><?php endforeach; ?></nav><div class="gmr-invitations__table-wrap"><table><thead><tr><th>Destinatario</th><th>Creada</th><th>Vence</th><th>Estado</th><th>Uso y actividad</th><th></th></tr></thead><tbody><?php if ( ! $invites ) : ?><tr><td colspan="6" class="gmr-invitations__empty">No hay invitaciones en este filtro.</td></tr><?php endif; foreach ( $invites as $invite ) : $state = self::state( $invite ); $user = $invite->used_by ? get_userdata( (int) $invite->used_by ) : null; ?><tr><td><strong><?php echo esc_html( $invite->invitee_name ?: 'Sin nombre asignado' ); ?></strong><small><?php echo esc_html( $invite->invitee_email ?: 'Correo libre' ); ?></small></td><td><?php echo esc_html( get_date_from_gmt( $invite->created_at, 'j M Y' ) ); ?></td><td><?php echo esc_html( get_date_from_gmt( $invite->expires_at, 'j M Y · H:i' ) ); ?></td><td><span class="gmr-invitation-status is-<?php echo esc_attr( $state ); ?>"><?php echo esc_html( array( 'active'=>'Pendiente', 'used'=>'Registrada', 'expired'=>'Vencida', 'revoked'=>'Revocada' )[ $state ] ); ?></span></td><td><?php if ( $user ) : ?><strong><?php echo esc_html( $user->display_name ); ?></strong><small><?php echo esc_html( 'Registro: ' . get_date_from_gmt( $invite->used_at, 'j M Y · H:i' ) ); ?></small><small><?php echo esc_html( (int) $invite->login_count . ' ingreso' . ( 1 === (int) $invite->login_count ? '' : 's' ) . ( $invite->last_login_at ? ' · último: ' . get_date_from_gmt( $invite->last_login_at, 'j M Y · H:i' ) : '' ) ); ?></small><?php else : ?><small>Aún sin registro</small><?php endif; ?></td><td><?php if ( 'active' === $state ) : ?><form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><input type="hidden" name="action" value="gmr_revoke_collector_invitation"><input type="hidden" name="invitation_id" value="<?php echo esc_attr( $invite->id ); ?>"><?php wp_nonce_field( 'gmr_revoke_collector_invitation_' . $invite->id ); ?><button class="button-link-delete" type="submit">Revocar</button></form><?php endif; ?></td></tr><?php endforeach; ?></tbody></table></div></section></div>
		</div><?php
	}
}
