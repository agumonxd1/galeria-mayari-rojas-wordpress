...
function gmr_theme_public_visibility_meta_query():array{return array('relation'=>'OR',array('key'=>'gmr_visibility','value'=>'public'),array('key'=>'gmr_visibility','compare'=>'NOT EXISTS'));}
...
function gmr_theme_elmar_discipline_image(string $slug):string{$fallbacks=array('pintura'=>'https://galeriamayarirojas.com/wp-content/uploads/elementor/thumbs/da883ca0-0bcc-47cc-acec-99bdfc6c6910-rq4gswgrr0p58qt4hip8csekfi2t9etopjrodzwe36.jpg','escultura'=>'https://galeriamayarirojas.com/wp-content/uploads/2025/06/WhatsApp-Image-2025-06-09-at-6.16.10-PM.jpeg','obra-grafica'=>'https://galeriamayarirojas.com/wp-content/uploads/2025/05/obra-grafica.jpg','joyeria'=>'https://galeriamayarirojas.com/wp-content/uploads/2025/05/JOYERIA.jpg');$term=get_term_by('slug',$slug,'product_cat');$category_image=$term?absint(get_term_meta($term->term_id,'thumbnail_id',true)):0;if($category_image&&($url=wp_get_attachment_image_url($category_image,'large')))return$url;$q=new WP_Query(array('post_type'=>'product','post_status'=>'publish','posts_per_page'=>1,'fields'=>'ids','orderby'=>'date','order'=>'DESC','tax_query'=>array('relation'=>'AND',array('taxonomy'=>'gmr_artist','field'=>'slug','terms'=>'elmar-rojas'),array('taxonomy'=>'product_cat','field'=>'slug','terms'=>$slug)),'meta_query'=>array(gmr_theme_public_visibility_meta_query(),array('key'=>'_thumbnail_id','compare'=>'EXISTS'))));return$q->posts?(get_the_post_thumbnail_url($q->posts[0],'large')?:($fallbacks[$slug]??'')):($fallbacks[$slug]??'');}
